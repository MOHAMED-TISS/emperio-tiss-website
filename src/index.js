...
async function resend(env, to, subject, html, replyTo) {
...
async function authAdmin(r, env) {
  return env.ADMIN_API_KEY && r.headers.get('authorization') === `Bearer ${env.ADMIN_API_KEY}`
}
async function confirmNewsletter(request, env) {
  ...
  await env.NEWS_DB.batch([env.NEWS_DB.prepare(
      "UPDATE subscribers SET confirmed_at=? WHERE email=?").bind(now(), row.email), env
...
async function subscribe(request, env) {
  ...
  await env.NEWS_DB.prepare(
    "INSERT INTO subscribers(email,language,consent_at,confirmed_at) VALUES(?,?,?,NULL) ON CONFLICT(email) DO UPDATE SET language=excluded.language,consent_at=excluded.consent_at,confirmed_at=NULL"
...
async function privateVerify(request, env) {
  ...
  const raw = clean(new URL(request.url).searchParams.get('token'), 200),
    ...
    row = await env.NEWS_DB.prepare(
      "SELECT email FROM auth_tokens WHERE token_hash=? AND type='private' AND expires_at>? ")
    .bind(hash, now()).first();
...
async function privateOffers(request, env) {
  ...
  const hash = await sha(m[1]),
    s = await env.NEWS_DB.prepare(
      "SELECT email FROM private_sessions WHERE token_hash=? AND expires_at>?").bind(hash, now())
    .first();
  if (!s) return json({
    ok: false,
    error: 'Unauthorized'
...
async function adminSendOffer(request, env) {
  ...
  const b = await request.json(),
    id = Number(b.offer_id),
    offer = await env.NEWS_DB.prepare("SELECT * FROM private_offers WHERE id=?").bind(id).first();
  if (!offer) return json({
    ok: false,
    error: 'Offer not found'
  }, 404);
  const clients = await env.NEWS_DB.prepare("SELECT email FROM clients WHERE status='approved'")
    .all();
  let sent = 0;
  for (const c of clients.results || []) {
    try {
      await resend(env, c.email, `EMPERIO PRIVATE — ${offer.title}`,
        `<h2>${offer.title}</h2><p><strong>${offer.category}</strong></p><p>Origin: ${offer.origin}<br>Destination: ${offer.destination}<br>Availability: ${offer.availability}</p><p>${offer.description||''}</p><p><a href="https://emperio-tiss.com/private/">View EMPERIO PRIVATE</a></p>`
        );
      sent++
    } catch {}
  }
  await env.NEWS_DB.prepare("UPDATE private_offers SET last_sent_at=? WHERE id=?").bind(now(), id)
    .run();
  return json({
    ok: true,
    sent
...
async function adminNewsletter(request, env) {
  ...
  const b = await request.json(),
    ...
    html = clean(b.html, 20000),
    language = clean(b.language, 2);
  if (!subject || !html) return json({
    ok: false,
    error: 'subject and html required'
  }, 400);
  const rows = language && ['es', 'en', 'fr', 'it', 'ar'].includes(language) ? await env.NEWS_DB
    .prepare("SELECT email FROM subscribers WHERE confirmed_at IS NOT NULL AND language=?").bind(
      language).all() : await env.NEWS_DB.prepare(
      "SELECT email FROM subscribers WHERE confirmed_at IS NOT NULL").all();
  let sent = 0;
  for (const c of rows.results || []) {
    try {
      await resend(env, c.email, subject, html);
      sent++
    } catch {}
  }
  return json({
