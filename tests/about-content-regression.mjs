...
for (const [lang, path, prefix] of pages) {
  ...
  assert.match(html, /Misión|Mission|Mission|Missione|رسالتنا/i, `${lang}: missing mission section`);
  assert.match(html, /Valores|Values|Valeurs|Valori|قيم/i, `${lang}: missing values section`);
  assert.match(html, /medio ambiente|environment|environnement|ambiente|البيئة/i, `${lang}: missing environmental responsibility section`);
  assert.match(html, /tecnolog|technology|technologie|tecnologia|التكنولوجيا/i, `${lang}: missing supporting technology reference`);
