(() => {
  ...
  const products = [
    ...
    ['bottarga',{en:'Frozen bottarga',fr:'Bottarga surgelée'},'According to product specification','special','Special','FROZEN','According to origin','According to FAO area']
  ...
  const ensureViewer = () => {
    ...
    const box = document.createElement('div'); box.id = 'intlFishViewer'; box.className = 'fish-gallery'; box.hidden = true;
  ...
  const details = p => [[labels.family,categoryNames[p.category]],[labels.type,p.type],[labels.state,p.condition.map(x => x === 'FRESH' ? labels.fresh : labels.frozen).join(' / ')],[labels.origin,p.origin],[labels.fao,p.fao],[labels.calibre,labels.according],[labels.quality,lang === 'fr' ? 'Spécification professionnelle' : 'Professional specification'],[labels.presentation,lang === 'fr' ? 'Selon destination' : 'According to destination'],[labels.packaging,lang === 'fr' ? 'Selon marché' : 'According to market'],[labels.availability,labels.according]].map(([k,v]) => `<div class="fish-catalog-card__detail"><span>${esc(k)}</span><strong>${esc(v)}</strong></div>`).join('');
  const bindMedia = (media, images) => { if (!images.length) return; const imageButton = media.querySelector('.fish-card-image-button'); const image = media.querySelector('.fish-card-image'); let current = 0; const counter = media.querySelector('.fish-card-counter'); const show = index => { current = (index + images.length) % images.length; if (image) image.src = images[current]; if (counter) counter.textContent = `${current + 1} / ${images.length}`; }; media.querySelector('.fish-card-nav--prev')?.addEventListener('click', e => { e.preventDefault(); e.stopPropagation(); show(current - 1); }); media.querySelector('.fish-card-nav--next')?.addEventListener('click', e => { e.preventDefault(); e.stopPropagation(); show(current + 1); }); imageButton?.addEventListener('click', e => { e.preventDefault(); openViewer(images); }); media.addEventListener('click', e => { if (e.target.closest('.fish-card-nav') || e.target.closest('.fish-card-image-button')) return; openViewer(images); }); };

  const renderEmblematic = () => { const target = document.getElementById('fishEmblematicGrid'); if (!target) return; target.closest('.fish-emblematic')?.querySelector('.fish-emblematic__intro p')?.replaceChildren(document.createTextNode(labels.emblemNote)); target.innerHTML = emblematic.map((p,i) => { const images = imageMap[p.id] || []; const image = images[0] || ''; return `<article class="fish-emblematic-card" data-product-id="${esc(p.id)}"><div class="fish-emblematic-card__media${image ? ' fish-emblematic-card__media--image' : ''}" data-images='${esc(JSON.stringify(images))}'>${image ? `<button type="button" class="fish-emblematic-card__image-button" aria-label="${esc(labels.zoom)} — ${esc(p.name)}"><img src="${esc(image)}" alt="${esc(p.name)}" loading="lazy" draggable="false"></button>` : '<span>EMPERIO TISS</span>'}${image ? `<span class="fish-emblematic-card__zoom-label">${labels.zoom}</span>` : ''}</div><div class="fish-emblematic-card__body"><span class="fish-emblematic-card__kicker">0${i+1} / ${labels.emblem}</span><h3>${esc(p.name)}</h3><p class="fish-emblematic-card__scientific"><em>${esc(p.scientific)}</em></p><div class="fish-emblematic-card__meta"><span>${esc(categoryNames[p.cat])}</span><span>${esc(lang === 'fr' ? 'Marché français' : 'English-speaking market')}</span></div><p class="fish-emblematic-card__note">${esc(lang === 'fr' ? 'Selon disponibilité · frais ou congelé selon référence' : 'According to availability · fresh or frozen by reference')}</p><span class="fish-emblematic-card__mark">${labels.professional}</span></div></article>`; }).join(''); target.querySelectorAll('.fish-emblematic-card__media--image').forEach(media => { const images = JSON.parse(media.dataset.images || '[]'); const button = media.querySelector('.fish-emblematic-card__image-button'); button?.addEventListener('click', e => { e.preventDefault(); openViewer(images); }); media.style.cursor = images.length ? 'zoom-in' : ''; }); };

  const render = () => { const q = String(search.value || '').trim().toLowerCase(); const visible = products.filter(p => { const cOk = condition === 'all' || p.condition.includes(condition); const catOk = category === 'all' || p.category === category; const hay = [p.name,p.names.en,p.names.fr,p.scientific,p.type,p.origin,p.fao,categoryNames[p.category]].join(' ').toLowerCase(); return cOk && catOk && (!q || hay.includes(q)); }); count.textContent = `${visible.length} ${visible.length === 1 ? labels.ref : labels.refs}`; grid.innerHTML = visible.length ? visible.map(p => { const images = imageMap[p.id] || []; const img = images[0] || ''; return `<article class="fish-catalog-card" data-product-id="${esc(p.id)}"><div class="fish-catalog-card__media" data-images='${esc(JSON.stringify(images))}'>${img ? `<button type="button" class="fish-card-image-button" aria-label="${esc(labels.zoom)} — ${esc(p.name)}"><img class="fish-card-image" src="${esc(img)}" alt="${esc(p.name)}" loading="lazy" draggable="false"></button>` : '<span>EMPERIO TISS</span>'}${images.length > 1 ? `<button class="fish-card-nav fish-card-nav--prev" type="button" aria-label="Previous image">‹</button><button class="fish-card-nav fish-card-nav--next" type="button" aria-label="Next image">›</button><span class="fish-card-counter">1 / ${images.length}</span>` : ''}</div><div class="fish-catalog-card__body"><p class="fish-catalog-card__meta">${esc(categoryNames[p.category])}</p><h3 class="fish-catalog-card__title">${esc(p.name)}</h3><p class="fish-catalog-card__scientific"><em>${esc(p.scientific)}</em></p><div class="fish-catalog-card__details">${details(p)}</div></div></article>`; }).join('') : `<p class="fish-catalog__empty">${labels.none}</p>`; grid.querySelectorAll('.fish-catalog-card__media').forEach(media => bindMedia(media, JSON.parse(media.dataset.images || '[]'))); };
