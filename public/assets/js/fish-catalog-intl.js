(() => {
  ...
  const renderEmblematic = () => {
    ...
    target.innerHTML=emblematic.map((p,i)=>{const images=imageMap[p.id]||[];const image=images[0]||'';return `<article class="fish-emblematic-card" data-product-id="${esc(p.id)}"><div class="fish-emblematic-card__media${image?' fish-emblematic-card__media--image':''}" data-images='${esc(JSON.stringify(images))}'>${image?`<img src="${esc(image)}" alt="${esc(p.name)}" loading="lazy" draggable="false">`:'<span>EMPERIO TISS</span>'}${image?`<span class="fish-emblematic-card__zoom-label">${labels.zoom}</span>`:''}</div><div class="fish-emblematic-card__body"><span class="fish-emblematic-card__kicker">0${i+1} / ${labels.emblem}</span><h3>${esc(p.name)}</h3><p class="fish-emblematic-card__scientific"><em>${esc(p.scientific)}</em></p><div class="fish-emblematic-card__meta"><span>${esc(categoryNames[p.cat])}</span><span>${esc(lang==='fr'?'Marché français':'English-speaking market')}</span></div><p class="fish-emblematic-card__note">${esc(lang==='fr'?'Selon disponibilité · frais ou congelé selon référence':'According to availability · fresh or frozen by reference')}</p><span class="fish-emblematic-card__mark">${labels.professional}</span></div></article>`;}).join('');
    target.querySelectorAll('.fish-emblematic-card__media--image').forEach(media=>{const images=JSON.parse(media.dataset.images||'[]');if(images.length)media.addEventListener('click',()=>openViewer(images));});
  };

  const render = () => {
    const q=String(search.value||'').trim().toLowerCase();
    const visible=products.filter(p=>{
      const cOk=condition==='all'||p.condition.includes(condition);const catOk=category==='all'||p.category===category;
