(() => {
  ...
  const set = (t, ok = false) => {
    if (status) {
      ...
      status.style.color = ok ? '#d7bb7d' : '#fff'
    }
  };
  const load = async () => {
    try {
      const r = await fetch('/api/private/offers', {
          credentials: 'include',
          headers: {
      ...
      if (!r.ok || !d.ok) {
        set('Private access is not active for this session. Use the access request on News.');
        return
      }
      ...
      set('Unable to load private offers.')
    }
  };
  load()
})();
