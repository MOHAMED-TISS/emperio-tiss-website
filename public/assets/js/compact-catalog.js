(() => {
  ...
  const dedicatedDataUrl = lang === 'en' && frozenSeafoodFamily
    ? (subcategories.includes('shellfish') ? '/assets/data/shellfish-catalog-es.json' : '/assets/data/cephalopods-catalog-es.json')
    : null;
  const enNames={
    moruno:'Mediterranean red shrimp',cigala:'Norway lobster',"gamba-blanca":'Deep-water rose shrimp',"langostino-tigre":'Tiger prawn',
    'pulpo-flor':'Flower octopus','pulpo-bloque':'Block octopus','calamar-envuelto':'Wrapped squid','sepia-limpia-iqf':'Cleaned cuttlefish IQF'
  };
  ...
  const loadData = async () => {
    if (dedicatedDataUrl) {
      const response=await fetch(dedicatedDataUrl,{cache:'no-cache'});
      if(!response.ok) throw new Error(`Catalogue request failed: ${response.status}`);
      const data=await response.json();
      return (data.products||[]).map(p=>({...p,condition:['frozen'],_enName:enNames[p.id]||p.commercialName,_enType:enTypes[p.id]||p.type||p.group}));
    }
