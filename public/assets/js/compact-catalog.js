(() => {
  ...
  const subcategories = (body?.dataset.catalogSubcategories || body?.dataset.catalogSubcategory ||
  ...
  const i18n = {
    es: {
      ...
      frozen: 'Congelado',
      detail: 'Ver ficha',
      empty: 'No hay referencias activas.'
    },
    en: {
      fresh: 'Fresh',
      frozen: 'Frozen',
      detail: 'View specification',
      empty: 'No active references.'
    },
    fr: {
      fresh: 'Frais',
      frozen: 'Surgelé',
      detail: 'Voir la fiche',
      empty: 'Aucune référence active.'
    },
    ar: {
      fresh: 'طازج',
      frozen: 'مجمد',
      detail: 'عرض المواصفات',
      empty: 'لا توجد مراجع نشطة'
    }
  };
  ...
  const frozenSeafoodFamily = family === 'seafood' && subcategories.some(s => s === 'shellfish' ||
    s === 'cephalopods');
  const dedicatedSubcategory = subcategories.find(s => s === 'shellfish' || s ===
    'cephalopods') || '';
  const dedicatedDataUrl = lang === 'en' && frozenSeafoodFamily ?
    (dedicatedSubcategory === 'shellfish' ? '/assets/data/shellfish-catalog-es.json' :
      '/assets/data/cephalopods-catalog-es.json') :
    null;
  const enNames = {
    moruno: 'Mediterranean red shrimp',
    cigala: 'Norway lobster',
    'gamba-blanca': 'Deep-water rose shrimp',
  ...
  const enTypes = {
    ...
    cigala: 'Mediterranean',
    'gamba-blanca': 'Mediterranean',
  ...
  lbPrev.addEventListener('click', () => moveGallery(-1));
  lbNext.addEventListener('click', () => moveGallery(1));
  ...
  function matches(product) {
    ...
    const hay = [product.commercialName, product.scientificName, first(product.origin), first(
        product.variety), first(product.campaign), first(product.quality), product._enName ||
      '', product._enType || ''
    ].join(' ').toLowerCase();
  ...
  function card(product) {
    ...
    const href =
      `${lang==='en'?'/en/products/product.html':'/products/product.html'}?${params.toString()}`;
  ...
  function syncFilterAvailability() {
    const hasFresh = products.some(p => (p.condition || []).includes('fresh'));
    const hasFrozen = products.some(p => (p.condition || []).includes('frozen'));
    filters.forEach(button => {
      const value = button.dataset.compactFilter || 'all';
      const available = value === 'all' || (value === 'fresh' ? hasFresh : hasFrozen);
      button.disabled = !available;
      button.setAttribute('aria-disabled', String(!available));
    });
    if (frozenSeafoodFamily) {
      const fresh = filters.find(b => b.dataset.compactFilter === 'fresh');
      if (fresh) {
        fresh.disabled = true;
        fresh.setAttribute('aria-disabled', 'true');
        fresh.title = t.fresh + ' not available for this catalogue';
      }
      const frozen = filters.find(b => b.dataset.compactFilter === 'frozen');
      if (frozen) {
        frozen.disabled = false;
        frozen.removeAttribute('title');
      }
      activeFilter = 'frozen';
      filters.forEach(item => item.setAttribute('aria-pressed', String((item.dataset
        .compactFilter || 'all') === activeFilter)));
    }
  }
  ...
  function render() {
    ...
    grid.innerHTML = visible.length ? visible.map(card).join('') :
      `<p class="compact-catalog__empty">${esc(t.empty)}</p>`;
    grid.querySelectorAll('[data-gallery]').forEach(button => button.addEventListener('click',
    () => showGallery(JSON.parse(button.dataset.gallery))));
  ...
  const loadData = async () => {
    if (dedicatedDataUrl) {
      const response = await fetch(dedicatedDataUrl, {
        cache: 'no-cache'
      });
      ...
      return (data.products || []).map(p => ({
        ...p,
        condition: ['frozen'],
        _enName: enNames[p.id] || p.commercialName,
        _enType: enTypes[p.id] || p.type || p.group
      }));
    }
    const [catalogResponse, imageResponse] = await Promise.all([fetch(
    ...
    return (data.products || []).filter(p => p.status === 'active' && p.family === family && (
  ...
  loadData().then(data => {
    ...
    console.error('[EMPERIO TISS] Catalogue failed', err);
    grid.innerHTML = `<p class="compact-catalog__empty">${esc(t.empty)}</p>`;
    count.textContent = '—';
  });
})();
