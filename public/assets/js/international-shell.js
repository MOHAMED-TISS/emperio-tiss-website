(() => {
  ...
  if (!initialHomeHeader) doc.querySelectorAll('body > header, .site-header').forEach(el => el.remove());
  doc.querySelectorAll('.nav-overlay,.p-overlay,footer').forEach(el => el.remove());
  doc.body.insertAdjacentHTML('afterbegin', initialHomeHeader ? overlay : header + overlay);
