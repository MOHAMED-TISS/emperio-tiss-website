(() => {
  ...
  const header = `<header class="site-header" id="luxuryHeader"><div class="header-inner"><a href="${href.home}" class="site-logo" aria-label="${cfg.homeAria}"><img src="/logo.png" alt="EMPERIO TISS" width="94" height="62"></a><a href="https://wa.me/34614270684" class="et-whatsapp" target="_blank" rel="noopener noreferrer" aria-label="${cfg.whatsapp}">${whatsappIcon}<span>${cfg.whatsapp}</span></a><nav class="et-language-switch" aria-label="${lang==='ar'?'اللغة':lang==='fr'?'Langue':'Language'}"><a href="/">ES</a><span>·</span><a href="/en/" ${lang==='en'?'class="current" aria-current="page"':''}>EN</a><span>·</span><a href="/fr/" ${lang==='fr'?'class="current" aria-current="page"':''}>FR</a><span>·</span><a href="/ar/" ${lang==='ar'?'class="current" aria-current="page"':''}>AR</a></nav><button id="menuToggleBtn" class="mobile-menu" type="button" aria-label="${cfg.open}" aria-expanded="false" aria-controls="navOverlay"><span></span><span></span><span></span></button></div></header>`;
