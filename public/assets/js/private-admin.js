(() => {
  ...
  const set = (el, msg, ok = false) => {
    if (!el) return;
    el.textContent = msg;
    el.style.color = ok ? '#b9d9bd' : '#d8bb7a'
  };
  ...
  bind('[data-offer-form]', async form => {
    ...
    await call('/api/private/admin/offers', {
    ...
    return 'Private offer published.'
  });
  bind('[data-send-offer-form]', async form => {
    ...
    const d = await call('/api/private/admin/send-offer', {
      offer_id: id
    });
    return `Sent to ${d.sent||0} approved client(s).`
  });
  bind('[data-newsletter-form]', async form => {
    const fd = new FormData(form);
    const d = await call('/api/admin/newsletter/send', {
      subject: String(fd.get('subject') || '').trim(),
      html: String(fd.get('html') || ''),
      language: String(fd.get('language') || '')
    });
    return `Market Signals sent to ${d.sent||0} subscriber(s).`
  });
})();
