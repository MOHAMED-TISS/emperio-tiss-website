(() => {
  ...
  const buildFish = () => {
    ...
    const makeType = (label, names) => {
      const d = group('nav-fish-type', label, []), b = d.querySelector('.nav-products-links');
  ...
  const normalize = links => {
    ...
    produceBox.appendChild(group('nav-fruits', nav.fruits, [nav.citrus, nav.exoticFruit, nav.otherFruit].map(leaf)));
  ...
  new MutationObserver(run).observe(body, { childList: true, subtree: true });
})();
