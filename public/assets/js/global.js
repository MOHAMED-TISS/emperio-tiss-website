(() => {
  ...
  loadCss('/assets/css/catalogue-type-scale-unified.css?v=20260823-es-baseline-2',
    'etCatalogueTypeScale');
  loadCss('/assets/css/catalogue-filter-contrast-en-fr.css?v=20260823-filter-contrast-1',
    'etCatalogueFilterContrast');
  loadCss('/assets/css/header-final.css?v=20260824-11', 'etHeaderFinalCanonical');

  if (['en', 'fr', 'ar', 'it'].includes(lang)) loadScript(
    '/assets/js/international-shell.js?v=20260824-5', 'etInternationalShell');
  ...
  const rebuildFishHeader = () => {
    ...
    const legacyStyle = Array.from(doc.querySelectorAll('style')).find((style) => style
      .textContent.includes('body.fish-catalog-pilot .site-header'));
    if (legacyStyle) {
      ...
      if (start >= 0 && end > start) legacyStyle.textContent =
  ...
  const italianHref = () => {
    ...
    for (const prefix of ['/en', '/fr', '/ar'])
      if (pathname === prefix || pathname.startsWith(`${prefix}/`)) pathname = pathname.slice(
        prefix.length) || '/';
  ...
  const ensureItalianLanguageLinks = () => {
    ...
    doc.querySelectorAll('.et-language-switch,.language-nav,.nav-overlay-lang').forEach((
      switcher) => {
      if (switcher.querySelector('a[href^="/it/"]')) return;
      const link = doc.createElement('a');
      link.href = href;
      link.textContent = 'IT';
      const trailing = Array.from(switcher.children).reverse().find((el) => el.tagName ===
        'A');
      if (trailing) {
  ...
  const ensureHeaderWhatsApp = () => {
    ...
    if (reference && link.nextElementSibling !== reference) container.insertBefore(link,
      reference);
  };
  ...
  const ensureFooterLinkedIn = () => {
    ...
    const bottom = footer.querySelector(
      '.et-footer-bottom,.et-footer-legal,.footer-band-inner,.ar-footer-inner');
    if (bottom) bottom.appendChild(link);
  ...
  const protectPageImages = (rootElement = doc) => rootElement.querySelectorAll(pageImageSelector)
    .forEach((img) => {
  ...
  doc.addEventListener('contextmenu', (event) => {
    if (event.target.closest(pageImageSelector)) event.preventDefault();
  }, true);
  ...
  new MutationObserver(() => protectPageImages()).observe(doc.documentElement, {
    childList: true,
    subtree: true
  });
