(() => {
  ...
  const removeLegacyArrows = () => {
    doc.querySelectorAll(
        'a,button,.product-arrow,.text-link,.es-link,.intl-link,.ar-link,.product-card__link')
      .forEach(el => {
  ...
  const ensureUniversalFooter = () => {
    ...
    const copy = lang === 'fr' ? {
      ...
      seasonal: 'Temporada',
      inquiry: 'Consulta empresarial',
      legal: 'Aviso legal',
      privacy: 'Política de privacidad',
      cookies: 'Política de cookies',
      reserved: 'Todos los derechos reservados.',
      disclaimer: 'La información publicada tiene carácter informativo y no constituye una oferta contractual.',
  ...
  const initSecureContactForm = () => {
      ...
      widgetId = window.turnstile.render(mount, {
        ...
        'expired-callback': () => {
          ...
          if (rejectToken) {
            ...
            resolveToken = null;
            rejectToken = null;
            reject(new Error('La verificación de seguridad ha caducado.'));
          }
        },
        'error-callback': (code) => {
          resolving = false;
    ...
    const getToken = async () => {
      ensureWidget();
      if (widgetId === null) throw new Error(
    ...
    form.addEventListener('submit', async (event) => {
      ...
      try {
        ...
        if (!input) {
          ...
          input.type = 'hidden';
          input.id = 'contactTurnstileToken';
        ...
        if (!response.ok || !result.ok) {
          const codes = Array.isArray(result.turnstileErrors) ? result.turnstileErrors
            .join(', ') : '';
          throw new Error((result.error || 'No se pudo enviar la consulta.') + (codes ?
            ` (${codes})` : ''));
        }
        ...
        const input = form.querySelector('#contactTurnstileToken');
