(() => {
  ...
  const ensureUniversalFooter = () => {
    ...
    // Keep the approved metallic logo preview confined to the Spanish homepage.
  ...
  const initSecureContactForm = () => {
    ...
    let widgetId = null, resolving = false, resolveToken = null, rejectToken = null;
    const ensureWidget = () => {
      if (widgetId !== null || !window.turnstile) return;
      widgetId = window.turnstile.render(mount, {
        sitekey: '0x4AAAAAAEaIn_beKLMv4VjA', action: 'contact', execution: 'execute', appearance: 'interaction-only',
        callback: token => { resolving = false; if (resolveToken) { const resolve = resolveToken; resolveToken = null; rejectToken = null; resolve(token); } },
        'expired-callback': () => { resolving = false; if (rejectToken) { const reject = rejectToken; resolveToken = null; rejectToken = null; reject(new Error('La verificación de seguridad ha caducado.')); } },
        'error-callback': code => { resolving = false; if (rejectToken) { const reject = rejectToken; resolveToken = null; rejectToken = null; reject(new Error(`Turnstile error: ${code || 'unknown'}`)); } },
      });
    };
    const getToken = async () => {
      ...
      resolving = true;
      return new Promise((resolve, reject) => { resolveToken = resolve; rejectToken = reject; window.turnstile.execute(widgetId); });
    };
    form.addEventListener('submit', async event => {
      event.preventDefault(); event.stopImmediatePropagation();
      if (!form.reportValidity() || button.disabled) return;
      button.disabled = true; status.textContent = 'Verificando seguridad…'; status.dataset.state = 'pending';
      try {
        ...
        input.value = token; status.textContent = 'Enviando consulta…';
        const response = await fetch('/api/contact', { method: 'POST', body: new FormData(form), headers: { Accept: 'application/json' } });
        const result = await response.json().catch(() => ({}));
        if (!response.ok || !result.ok) { const codes = Array.isArray(result.turnstileErrors) ? result.turnstileErrors.join(', ') : ''; throw new Error((result.error || 'No se pudo enviar la consulta.') + (codes ? ` (${codes})` : '')); }
        status.textContent = 'Consulta enviada correctamente. Gracias.'; status.dataset.state = 'success'; form.reset(); input.value = ''; if (widgetId !== null) window.turnstile.reset(widgetId);
      } catch (error) {
        status.textContent = error.message || 'No se pudo enviar la consulta. Inténtalo de nuevo.'; status.dataset.state = 'error'; const input = form.querySelector('#contactTurnstileToken'); if (input) input.value = ''; if (widgetId !== null) window.turnstile.reset(widgetId);
      } finally { button.disabled = false; }
    }, true);
    ensureWidget();
  };
  const bootContact = () => { if (window.turnstile) initSecureContactForm(); else window.setTimeout(bootContact, 200); };
  if (window.location.pathname === '/contact/') { bootContact(); window.addEventListener('load', bootContact, { once: true }); }
})();
