(() => {
  ...
  style.textContent=`
    ...
    :is(.text-link,.es-link,.intl-link,.ar-link,.product-card__link) > span:last-child,
    .product-arrow{font-size:0!important;line-height:0!important;width:1.2em;min-width:1.2em;display:inline-flex;align-items:center;justify-content:center}
    :is(.button,.es-btn,.intl-btn,.ar-btn,.about-btn,.cta-button,.contact-button,.inquiry-button) > span:last-child::before,
    :is(.text-link,.es-link,.intl-link,.ar-link,.product-card__link) > span:last-child::before,
    .product-arrow::after{content:"";display:block;width:11px;height:7px;background:currentColor;clip-path:polygon(0 43%,78% 43%,58% 0,100% 50%,58% 100%,78% 57%,0 57%);transition:transform .28s cubic-bezier(.165,.84,.44,1)}
    :is(.button,.es-btn,.intl-btn,.ar-btn,.about-btn,.cta-button,.contact-button,.inquiry-button):hover > span:last-child::before,
    :is(.text-link,.es-link,.intl-link,.ar-link,.product-card__link):hover > span:last-child::before,
    .product-row:hover .product-arrow::after{transform:translateX(4px)}
    .home-page .button{min-height:52px;padding-inline:22px}
    .et-header-inner,.header-inner{backface-visibility:hidden;-webkit-backface-visibility:hidden;transform:translateZ(0)}
  ...
  const ensureUniversalFooter=()=>{
    ...
    const copy=isFr ? {
      ...
      reserved:'Tous droits réservés.',disclaimer:'Les informations publiées sont fournies à titre informatif et ne constituent pas une offre contractuelle.',region:'MADRID · ESPAGNE · EUROPE · AFRIQUE · MÉDITERRANÉE'
    } : {
      ...
      reserved:'Todos los derechos reservados.',disclaimer:'La información publicada tiene carácter informativo y no constituye una oferta contractual.',region:'MADRID · ESPAÑA · EUROPA · ÁFRICA · MEDITERRÁNEO'
    };
    const base=isFr?'/fr/':'/';
