(() => {
  ...
  const i18n = {
    ...
    fr: {labels:{fish:'Poissons',shellfish:'Fruits de mer / Crustacés',cephalopods:'Céphalopodes',citrus:'Agrumes',exotics:'Fruits exotiques','core-produce':'Autres fruits',vegetables:'Légumes','seasonal-selection':'Saisonnier',mediterranean:'Méditerranée'},fresh:'Frais',frozen:'Surgelé',detail:'Voir la fiche',empty:'Aucune référence active dans cette catégorie.',catalogue:'CATALOGUE',all:'Références sélectionnées.',seafood:'Produits de la mer',seasonal:'Saisonnier'},
  ...
  function autoCatalogTarget(){
    ...
    }else if(rest[0]==='seasonal'){family='seasonal';subcategories=['seasonal-selection'];title=t.seasonal;}
    if(!family)return null;
    const main=document.querySelector('main');if(!main)return null;
    const section=document.createElement('section');section.className='catalog-section auto-catalog-section';
    ...
    main.appendChild(section);return section.querySelector('[data-catalog-family]');
  }
  async function init(){
    const injected=autoCatalogTarget();const targets=document.querySelectorAll('[data-catalog-family]');if(!targets.length&&!injected)return;
    try{const data=await loadCatalog();renderRequestedCatalogs(data);document.documentElement.dataset.catalogReady='true';}
    catch(error){console.error('[EMPERIO TISS] Catalog load failed:',error);document.querySelectorAll('[data-catalog-family]').forEach((element)=>{element.innerHTML=`<p class="catalog-empty">${esc(t.empty)}</p>`;});document.documentElement.dataset.catalogReady='false';}
  }
