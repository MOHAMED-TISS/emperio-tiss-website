(() => {
  ...
  function ensureLightbox(){
    ...
    const box=document.createElement('div');box.id='etProductLightbox';box.setAttribute('aria-hidden','true');
  ...
  function showLightbox(index){if(!lightboxImages.length)return;lightboxIndex=(index+lightboxImages.length)%lightboxImages.length;const box=document.getElementById('etProductLightbox');const image=box.querySelector('.et-lb-image');image.src=lightboxImages[lightboxIndex];image.alt=image.dataset.productName||'';box.querySelector('.et-lb-count').textContent=`${lightboxIndex+1} / ${lightboxImages.length}`;box.querySelector('.et-lb-prev').hidden=lightboxImages.length<2;box.querySelector('.et-lb-next').hidden=lightboxImages.length<2;const thumbs=box.querySelector('.et-lb-thumbs');thumbs.replaceChildren();if(lightboxImages.length>1)lightboxImages.forEach((src,i)=>{const button=document.createElement('button');button.type='button';button.className='et-lb-thumb';button.setAttribute('aria-label',`${i+1}`);button.innerHTML=`<img src="${esc(src)}" alt="">`;button.addEventListener('click',()=>showLightbox(i));thumbs.appendChild(button);});}
  ...
  async function loadCatalog(){
    const response=await fetch(CATALOG_URL,{cache:'no-cache'});if(!response.ok)throw new Error(`Catalog request failed: ${response.status}`);const data=await response.json();if(!data||!Array.isArray(data.products))throw new Error('Invalid catalog schema');
    if(lang==='es' && window.location.pathname.includes('/products/seafood/cephalopods/')){
      const cephResponse=await fetch(CEPHALOPOD_ES_URL,{cache:'no-cache'});if(!cephResponse.ok)throw new Error(`Cephalopod catalogue request failed: ${cephResponse.status}`);const ceph=await cephResponse.json();if(Array.isArray(ceph.products)){data.products=data.products.filter(product=>product.subcategory!=='cephalopods');data.products.push(...ceph.products.map(product=>({...product,family:'seafood',category:'seafood',subcategory:'cephalopods',status:'active'})));}
    }
    return data;
  }
