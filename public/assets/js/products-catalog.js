(() => {
  ...
  const CEPHALOPOD_ES_URL = '/assets/data/cephalopods-catalog-es.json';
  const i18n = {
    es: {
      labels: {
        fish: 'Pescados',
        ...
        mediterranean: 'Del Mediterráneo'
      },
      fresh: 'Fresco',
      frozen: 'Congelado',
      detail: 'Ver ficha',
      empty: 'No hay referencias activas en esta categoría.',
      catalogue: 'CATÁLOGO',
      all: 'Referencias seleccionadas.',
    ...
    en: {
      ...
      empty: 'No active references in this category.',
      catalogue: 'CATALOGUE',
    ...
    fr: {
      labels: {
        fish: 'Poissons',
        shellfish: 'Fruits de mer / Crustacés',
        cephalopods: 'Céphalopodes',
        citrus: 'Agrumes',
        exotics: 'Fruits exotiques',
        'core-produce': 'Autres fruits',
        'vegetables': 'Légumes',
        'seasonal-selection': 'Saisonnier',
        mediterranean: 'Méditerranée'
      },
      fresh: 'Frais',
      frozen: 'Surgelé',
      detail: 'Voir la fiche',
      empty: 'Aucune référence active dans cette catégorie.',
      catalogue: 'CATALOGUE',
      all: 'Références sélectionnées.',
  ...
  const naturalImageCompare = (a, b) => {
    ...
    const parse = nameValue => {
      ...
      return {
        base: (match?.[1] || nameValue).trim().toLocaleLowerCase(),
  ...
  const images = (product) => [...new Set((Array.isArray(product.images) ? product.images : (
    product.image ? [product.image] : [])).filter(Boolean))].sort(naturalImageCompare);
  const normalizeCondition = (values) => (values || []).map((value) => value === 'fresh' ? t
    .fresh : value === 'frozen' ? t.frozen : value).join(' · ');
  ...
  function showLightbox(index) {
    ...
    box.querySelector('.et-lb-count').textContent =
    ...
    const thumbs = box.querySelector('.et-lb-thumbs');
    thumbs.replaceChildren();
    if (lightboxImages.length > 1) lightboxImages.forEach((src, i) => {
      const button = document.createElement('button');
      button.type = 'button';
      button.className = 'et-lb-thumb';
      button.setAttribute('aria-label', `${i+1}`);
      button.innerHTML = `<img src="${esc(src)}" alt="">`;
  ...
  async function loadCatalog() {
    ...
    if (lang === 'es' && window.location.pathname.includes('/products/seafood/cephalopods/')) {
      const cephResponse = await fetch(CEPHALOPOD_ES_URL, {
        cache: 'no-cache'
      });
      if (!cephResponse.ok) throw new Error(
        `Cephalopod catalogue request failed: ${cephResponse.status}`);
      const ceph = await cephResponse.json();
      if (Array.isArray(ceph.products)) {
        data.products = data.products.filter(product => product.subcategory !== 'cephalopods');
        data.products.push(...ceph.products.map(product => ({
          ...product,
          family: 'seafood',
          category: 'seafood',
  ...
  function bindGalleries(root = document) {
    root.querySelectorAll('[data-gallery-product]').forEach(button => {
      if (button.dataset.galleryBound) return;
      button.dataset.galleryBound = 'true';
      button.addEventListener('click', () => {
        const id = button.dataset.galleryProduct;
        const product = window.__ET_CATALOG_PRODUCTS?.find(item => item.id === id);
  ...
  function renderInto(element, products) {
    if (!element) return;
    element.innerHTML = products.length ? products.map(card).join('') :
      `<p class="catalog-empty">${esc(t.empty)}</p>`;
  ...
  function autoCatalogTarget() {
    ...
    if (rest[0] === 'seafood') {
      ...
      if (rest[1] === 'fish') {
        subcategories = ['fish'];
        title = t.labels.fish;
      } else if (rest[1] === 'shellfish') {
        subcategories = ['shellfish'];
        title = t.labels.shellfish;
      } else if (rest[1] === 'cephalopods') {
        subcategories = ['cephalopods'];
        title = t.labels.cephalopods;
      } else title = t.seafood;
    } else if (rest[0] === 'seasonal') {
      family = 'seasonal';
      subcategories = ['seasonal-selection'];
      title = t.seasonal;
    }
    ...
    section.innerHTML =
      `<div class="catalog-inner"><div class="catalog-head"><div><span class="eyebrow">${esc(eyebrow)}</span><h2>${esc(title)}<br><em>${esc(t.all)}</em></h2></div><p>${esc(t.detail)}</p></div><div class="product-catalog-grid" data-catalog-family="${esc(family)}" data-catalog-subcategories="${esc(subcategories.join(','))}"></div></div>`;
  ...
  async function init() {
    ...
    try {
      ...
      document.querySelectorAll('[data-catalog-family]').forEach(element => {
        element.innerHTML = `<p class="catalog-empty">${esc(t.empty)}</p>`;
      });
      document.documentElement.dataset.catalogReady = 'false';
    }
  }
  window.EMPERIO_TISS_CATALOG = {
    loadCatalog,
    renderInto,
    card,
    naturalImageCompare
