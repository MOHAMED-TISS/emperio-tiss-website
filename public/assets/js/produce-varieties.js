(() => {
  ...
  const availabilityLabels = {
    es: {
      trigger: 'Disponibilidad anual',
      spain: 'España',
      morocco: 'Marruecos',
      legend: 'Orientativo · sujeto a campaña y programa de suministro',
      high: 'Alta',
      medium: 'Media',
      limited: 'Limitada'
    },
    en: {
      trigger: 'Annual availability',
      spain: 'Spain',
      morocco: 'Morocco',
      legend: 'Indicative · subject to season and supply programme',
      high: 'High',
      medium: 'Medium',
      limited: 'Limited'
    },
    fr: {
      trigger: 'Disponibilité annuelle',
      spain: 'Espagne',
      morocco: 'Maroc',
      legend: 'Indicatif · selon campagne et programme d’approvisionnement',
      high: 'Forte',
      medium: 'Moyenne',
      limited: 'Limitée'
    },
    ar: {
      trigger: 'التوفر السنوي',
      spain: 'إسبانيا',
      morocco: 'المغرب',
      legend: 'إرشادي · حسب الموسم وبرنامج التوريد',
      high: 'مرتفع',
      medium: 'متوسط',
      limited: 'محدود'
    },
    it: {
      trigger: 'Disponibilità annuale',
      spain: 'Spagna',
      morocco: 'Marocco',
      legend: 'Indicativo · secondo campagna e programma di fornitura',
      high: 'Alta',
      medium: 'Media',
      limited: 'Limitata'
    }
  ...
  const requestLabels = {
    es: 'Solicitar referencia',
    en: 'Request reference',
    fr: 'Demander la référence',
    ar: 'طلب المرجع',
    it: 'Richiedi referenza'
  };
  ...
  async function waitForBaseCatalog(attempt = 0) {
    if (window.__ET_CATALOG_PRODUCTS && window.EMPERIO_TISS_CATALOG) return window
      .__ET_CATALOG_PRODUCTS;
  ...
  function availabilityPanel(product) {
    ...
    const legend =
      `<span class="availability-legend-item"><i class="is-high"></i>${safe(av.high)}</span><span class="availability-legend-item"><i class="is-medium"></i>${safe(av.medium)}</span><span class="availability-legend-item"><i class="is-limited"></i>${safe(av.limited)}</span>`;
    const body = rows.map(row => {
      const values = Array.isArray(product.availability[row.key]) ? product.availability[row
        .key] : [];
  ...
  function enrichCards(products, additions) {
    document.querySelectorAll('[data-product-id]').forEach(card => {
      ...
      if (!body) return;
      if (Array.isArray(product.varieties) && product.varieties.length && !body.querySelector(
        ...
        if (title) {
          ...
          line.innerHTML =
            `<span>${labels[lang] || labels.es}</span> ${product.varieties.map(safe).join(' · ')}`;
      ...
      if (product.availability && !body.querySelector('.product-card__availability')) {
        ...
        const fragment = document.createRange().createContextualFragment(availabilityPanel(
          product));
  ...
  async function init() {
    ...
    try {
      ...
      const merged = base.map(product => byId.has(product.id) ? {
        ...product,
        ...byId.get(product.id)
      } : product);
      const existingIds = new Set(base.map(product => product.id));
      const additions = extension.products.filter(product => !existingIds.has(product.id)).map(
        product => ({
          family: 'produce',
          category: 'produce',
          status: 'active',
          condition: ['fresh'],
          calibre: ['Según especificación del comprador'],
          image: '',
          images: [],
          ...product
        }));
