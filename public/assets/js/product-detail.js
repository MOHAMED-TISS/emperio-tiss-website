(() => {
  ...
  const requestedLang = (params.get('lang') || document.documentElement.lang || 'es').slice(0, 2)
    .toLowerCase();
  ...
  const dictionaries = {
    ...
    en: {
      ...
      missing: 'No product specified.',
      unavailable: 'Product unavailable.',
      error: 'The product specification could not be loaded.',
      request: 'Request this reference'
    },
    fr: {
      labels: {
        condition: 'Condition',
      ...
      missing: 'Produit non spécifié.',
      unavailable: 'Produit indisponible.',
      error: 'Impossible de charger la fiche produit.',
      request: 'Demander cette référence'
    },
    ar: {
      labels: {
        condition: 'الحالة',
      ...
      error: 'تعذر تحميل مواصفات المنتج.',
      request: 'طلب هذه المرجعية'
    }
  };

  const lang = dictionaries[requestedLang] ? requestedLang : 'es';
  const t = dictionaries[lang];
  const order = ['condition', 'origin', 'faoZone', 'calibre', 'quality', 'format', 'packaging',
    'availability', 'variety', 'campaign', 'brix', 'maturity', 'glazing', 'processing',
    'freezing', 'harvest', 'destination'
  ];
  ...
  const translateEnValue = value => {
    ...
    return Array.isArray(value) ? value.filter(Boolean).map(translateOne).join(' · ') :
      translateOne(value);
  };

  const pretty = (key, value) => {
    if (key === 'condition') return (Array.isArray(value) ? value : [value]).map(item =>
      item === 'fresh' ? t.fresh : item === 'frozen' ? t.frozen : item).join(' · ');
    return lang === 'en' && source ? translateEnValue(value) : Array.isArray(value) ? value
      .filter(Boolean).join(' · ') : String(value ?? '');
  };

  const backHref = lang === 'en' ? '/en/products/' : lang === 'fr' ? '/fr/products/' : lang ===
    'ar' ? '/ar/products/' : '/products/';
  ...
  async function getCatalog() {
    const url = dedicatedUrls[source] || '/assets/data/catalog-v1.3.json';
  ...
  async function enrichProduceProduct(product) {
    ...
    try {
      const response = await fetch('/assets/data/produce-varieties.json', {
        cache: 'no-cache'
      });
      if (!response.ok) return product;
      const extension = await response.json();
      const addition = Array.isArray(extension.products) ? extension.products.find(item => item
        .id === product.id) : null;
  ...
  async function render() {
    ...
    if (!id) {
      root.innerHTML =
        `<div class="product-detail-error"><h1>${esc(t.missing)}</h1><a href="${backHref}">${esc(t.back)}</a></div>`;
      return;
    }
    const data = await getCatalog();
    const product = data.products.find(item => item.id === id && (source ? true : item
      .status === 'active'));
    if (!product) {
      root.innerHTML =
        `<div class="product-detail-error"><h1>${esc(t.unavailable)}</h1><a href="${backHref}">${esc(t.back)}</a></div>`;
      return;
    }
    ...
    const category = product.type || product.subcategory || product.group || product.family ||
      'product';
    const specs = order.filter(key => product[key] && ((Array.isArray(product[key]) && product[
  ...
  render().catch(error => {
    ...
    root.innerHTML =
      `<div class="product-detail-error"><h1>${esc(t.error)}</h1><a href="${backHref}">${esc(t.back)}</a></div>`;
  });
})();
