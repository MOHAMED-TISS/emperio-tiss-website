(() => {
  ...
  async function render(){
    ...
    if(!id){ root.innerHTML=`<div class="product-detail-error"><h1>${esc(t.missing)}</h1><a href="/products/">${esc(t.back)}</a></div>`; return; }
    const data = await getCatalog();
    const product = data.products.find((item)=>item.id===id && item.status==='active');
    if(!product){ root.innerHTML=`<div class="product-detail-error"><h1>${esc(t.unavailable)}</h1><a href="/products/">${esc(t.back)}</a></div>`; return; }
