(() => {
  ...
  const labels = {
    ...
    ref: 'referenza',
    refs: 'referenze',
    empty: 'Nessuna referenza corrisponde alla ricerca.',
  ...
  const state = v => (Array.isArray(v) ? v : []).map(x => x === 'fresh' ? labels.fresh : x ===
    'frozen' ? labels.frozen : x).join(' · ');
  ...
  viewer.className = 'it-seafood-gallery';
  viewer.hidden = true;
  viewer.innerHTML =
    '<div class="it-seafood-gallery__panel"><img class="it-seafood-gallery__image" alt="" draggable="false"><button class="it-seafood-gallery__prev" type="button" aria-label="' +
    labels.prev +
    '">‹</button><button class="it-seafood-gallery__next" type="button" aria-label="' + labels
    .next + '">›</button><button class="it-seafood-gallery__close" type="button" aria-label="' +
    labels.close + '">×</button><span class="it-seafood-gallery__count"></span></div>';
  document.body.appendChild(viewer);
  const vImg = viewer.querySelector('.it-seafood-gallery__image'),
    vCount = viewer.querySelector('.it-seafood-gallery__count');
  let gallery = [],
    gi = 0;
  const show = () => {
    vImg.src = gallery[gi];
    vCount.textContent = `${gi+1} / ${gallery.length}`;
    viewer.querySelector('.it-seafood-gallery__prev').hidden = gallery.length < 2;
    viewer.querySelector('.it-seafood-gallery__next').hidden = gallery.length < 2
  };
  const close = () => {
    viewer.hidden = true;
  ...
  viewer.querySelector('.it-seafood-gallery__prev').onclick = () => {
    gi = (gi - 1 + gallery.length) % gallery.length;
    show()
  };
  viewer.querySelector('.it-seafood-gallery__next').onclick = () => {
  ...
  const render = () => {
    ...
    grid.innerHTML = visible.length ? visible.map(card).join('') :
      `<p class="seafood-catalog-empty">${labels.empty}</p>`
  };
  grid.addEventListener('click', e => {
    const media = e.target.closest('.seafood-catalog-card__media');
    if (!media) return;
    try {
      open(JSON.parse(media.dataset.images || '[]'))
    } catch (_) {}
  });
  ...
  fetch(url, {
    ...
    count.textContent = '0 ' + labels.refs;
    grid.innerHTML = `<p class="seafood-catalog-empty">${labels.empty}</p>`
  });
})();
