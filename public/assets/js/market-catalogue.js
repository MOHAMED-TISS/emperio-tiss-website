(() => {
  ...
  Promise.all([fetch(CATALOG_URL, {
    ...
    .then(([catalog, priority, imageMap]) => {
      ...
      products.sort((a, b) => {
        ...
        return String(a.commercialName || a.id).localeCompare(String(b.commercialName || b
          .id), undefined, {
          numeric: true,
          sensitivity: 'base'
        });
  ...
  function render(products, priority, imageMap, orderIndex) {
    ...
    const conditionLabel = p => (p.condition || []).map(c => norm(c) === 'fresh' ? labels.fresh :
      norm(c) === 'frozen' ? labels.frozen : c).join(' / ') || labels.according;
    const details = p => [
      [labels.family, categoryLabel(p)],
      [labels.condition, conditionLabel(p)],
      [labels.origin, (p.origin || []).join(' / ') || labels.according],
      [labels.calibre, (p.calibre || []).join(' / ') || labels.according],
      [labels.quality, (p.quality || []).join(' / ') || labels.according],
      [labels.format, (p.format || []).join(' / ') || labels.according],
      [labels.packaging, (p.packaging || []).join(' / ') || labels.according],
      [labels.availability, (p.availability || []).join(' / ') || labels.according]
    ].map(([k, v]) =>
    ...
    const renderCards = () => {
      ...
      const visible = products.filter(p => {
        ...
        const hay = [translatedName(p), p.commercialName, p.scientificName, ...(p.origin ||
          []), ...(p.quality || [])].join(' ').toLowerCase();
        return cOk && (!q || hay.includes(q))
      });
      count.textContent =
        `${visible.length} ${visible.length===1?labels.reference:labels.references}`;
      grid.innerHTML = visible.length ? visible.map((p, index) => {
          const images = getImages(imageMap, p.id);
          const img = images[0] || p.image || '';
          const priorityBadge = orderIndex.has(p.id) ?
        ...
        `<p class="market-catalogue__empty">${lang==='ar'?'لا توجد مراجع مطابقة.':lang==='fr'?'Aucune référence ne correspond.':lang==='it'?'Nessuna referenza corrisponde.':'No references match your search.'}`;
      bindLightboxes();
    };
    const bindLightboxes = () => grid.querySelectorAll('.market-catalogue-card__media').forEach(
      media => {
    ...
    const openLightbox = images => {
      ...
      box.innerHTML =
        `<img src="${esc(images[0])}" alt=""><button class="market-catalogue__lb-close" type="button">×</button><button class="market-catalogue__lb-prev" type="button">‹</button><button class="market-catalogue__lb-next" type="button">›</button><span class="market-catalogue__lb-counter">1 / ${images.length}</span>`;
      document.body.appendChild(box);
      document.body.style.overflow = 'hidden';
      const img = box.querySelector('img'),
        counter = box.querySelector('.market-catalogue__lb-counter');
      const update = () => {
        img.src = images[i];
    ...
    section.querySelectorAll('.market-catalogue__filter').forEach(button => button
      .addEventListener('click', () => {
        state = button.dataset.state || 'all';
        section.querySelectorAll('.market-catalogue__filter').forEach(b => b.setAttribute(
          'aria-pressed', String(b === button)));
        renderCards()
      }));
