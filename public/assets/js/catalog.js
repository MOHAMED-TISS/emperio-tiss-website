/* EMPERIO TISS — canonical product catalogue renderer + interactions */
(() => {
  'use strict';
  ...
  const i18n = {
    ...
    it: {labels:{fish:'Pesce',shellfish:'Molluschi / Crostacei',cephalopods:'Cefalopodi',citrus:'Agrumi',exotics:'Frutta esotica','core-produce':'Altra frutta',vegetables:'Ortaggi','seasonal-selection':'Stagionale',mediterranean:'Mediterraneo'},fresh:'Fresco',frozen:'Surgelato',detail:'Vedi scheda',empty:'Nessuna referenza attiva in questa categoria.',catalogue:'CATALOGO',all:'Referenze selezionate.',seafood:'Prodotti del mare',seasonal:'Stagionale',previous:'Immagine precedente',next:'Immagine successiva',close:'Chiudi'}
  };
  ...
  function showLightbox(index) {
    ...
    if (lightboxImages.length > 1) lightboxImages.forEach((src, i) => {
      ...
      button.type = 'button'; button.className = 'et-lb-thumb'; button.setAttribute('aria-label', `${i + 1}`);
  ...
  function card(product) {
    ...
    const href = `${lang === 'es' ? '' : `/${lang}`}/products/product.html?id=${encodeURIComponent(product.id)}`;
  ...
  async function loadCatalog() {
    ...
    if (lang === 'es' && window.location.pathname.includes('/products/seafood/cephalopods/')) {
      const cephResponse = await fetch(CEPHALOPOD_ES_URL, { cache: 'no-cache' });
      if (!cephResponse.ok) throw new Error(`Cephalopod catalogue request failed: ${cephResponse.status}`);
      ...
      if (Array.isArray(ceph.products)) {
        ...
        data.products.push(...ceph.products.map((product) => ({ ...product, family: 'seafood', category: 'seafood', subcategory: 'cephalopods', status: 'active' })));
      }
    }
    return data;
  }
  ...
  function renderInto(element, products) {
    if (!element) return;
    element.innerHTML = products.length ? products.map(card).join('') : `<p class="catalog-empty">${esc(t.empty)}</p>`;
  ...
  document.addEventListener('click', (event) => {
    const filter = event.target.closest('.catalog-filter,[data-catalog-filter]');
    if (!filter) return;
  ...
  document.addEventListener('dragstart', (event) => {
    ...
    if (image && image.closest('.fish-catalog-card, .catalog-card, .catalog-product, .fish-gallery, [data-catalog]')) event.preventDefault();
  }, true);
